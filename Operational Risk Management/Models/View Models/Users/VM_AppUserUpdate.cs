...
{
    public record VM_AppUserUpdate : IValidationModel
    {
        [Required]
    ...
    /// <summary>
    /// Update AppUser model validation
    /// </summary>
    public class VLD_AppUserUpdate : AbstractValidatorWithDbContext<VM_AppUserUpdate>
    {
        public VLD_AppUserUpdate(HttpContext context) : base(context)
        {
            RuleFor(a => a.Id).Cascade(CascadeMode.StopOnFirstFailure).NotNull().Must((model,id) => _dbContext.ApplicationUsers.IsExists(model.Id.ToGuid())).WithMessage("Invalid request");
            RuleFor(a => a.UserName).Cascade(CascadeMode.StopOnFirstFailure).NotNull().Must((model,username) => _dbContext.ApplicationUsers.IsUnique(model.Id.ToGuid(),"UserName", username)).WithMessage("User name already exists");
